...
window.setInterval( ->
	...
	if (currPlayer.player.currentTime >= 228.5 and currPlayer.player.currentTime <= 230) or currPlayer.player.currentTime >= 450 and !intermediaryStage
...
forwardScene.on Events.Tap, (event) ->
	...
	if xCoord >= 600 and xCoord <= 700 and yCoord >= 300 and yCoord <= 400 
		if currTime >= 226 and currTime <= currTime <230
			switchPOV(videoLayerOne, videoLayerTwo, 232)
		if  currTime > 450
