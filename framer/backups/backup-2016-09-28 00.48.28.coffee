...
window.setInterval( ->
	...
	# loop while on ending scene
	if (currPlayer.player.currentTime >= 229 and currPlayer.player.currentTime <= 230) and !intermediaryStage
		currPlayer.player.currentTime = 227.7
		
	# jump to ending scene if reach 2nd POV ending
	if currPlayer.player.currentTime >= 450 and !intermediaryStage
		forwardToPlayerOne = true
		switchPOV(videoLayerTwo, videoLayerOne, 226)
		activePlayerIndex = 0
, 500)
